...
function make_color_video()
{
  color=$1
  resolution=$2
  ffmpeg -y -f lavfi -i nullsrc -filter_complex color=${color}:s=${resolution}x${resolution} -r 30 -frames:v 3000 -c:v libx264 -preset fast -g 25 -movflags faststart+frag_keyframe+empty_moov+default_base_moof -tune fastdecode ${color}.mp4
